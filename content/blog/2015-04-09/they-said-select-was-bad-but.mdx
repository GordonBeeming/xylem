...
```sql title="Create sample database"
USE [master]
GO
...
```sql title="Create view with SELECT *"
...
```sql title="Add column C to table"
...
```sql title="Check view definition"
...
```sql title="Recreate with 3 columns"
...
```sql title="Reproduce the weirdness"
-- drop helper objects if they exist
IF OBJECT_ID('dbo.vw_Test') IS NOT NULL
BEGIN
DROP VIEW dbo.vw_Test
...
```sql title="ALTER view to refresh"
-- alter view
...
```sql title="Check view column metadata"
...
```sql title="Refresh view metadata"
...
```sql title="Rename column A to D"
--rename column
...
```sql title="Check metadata after rename"
...
```sql title="Query sys.views and sys.columns"
SELECT c.*
FROM sys.views AS v
...
```sql title="Refresh and inspect metadata"
sp_refreshview 'dbo.vw_Test'
GO
...
```sql title="Drop column B, add column E"
ALTER TABLE dbo.tb_View_Test
DROP COLUMN B
GO
...
```sql title="Check metadata after drop/add"
SELECT ORDINAL_POSITION, *
FROM INFORMATION_SCHEMA.VIEW_COLUMN_USAGE AS cu
JOIN INFORMATION_SCHEMA.COLUMNS AS c
ON c.TABLE_SCHEMA = cu.TABLE_SCHEMA
...
```sql title="Refresh view and select"
sp_refreshview 'dbo.vw_Test'
GO
...
```sql title="Repopulate table with data"
...
```sql title="Drop C, add F, rename D to G"
ALTER TABLE dbo.tb_View_Test
DROP COLUMN C
GO
...
```sql title="Inspect metadata after rename"
SELECT ORDINAL_POSITION, *
FROM INFORMATION_SCHEMA.VIEW_COLUMN_USAGE AS cu
JOIN INFORMATION_SCHEMA.COLUMNS AS c
ON c.TABLE_SCHEMA = cu.TABLE_SCHEMA
...
```sql title="Refresh and verify data"
sp_refreshview 'dbo.vw_Test'
GO
...
```sql title="Drop column E and check metadata"
ALTER TABLE dbo.tb_View_Test
DROP COLUMN E
GO
...
```sql title="Reset to initial state"
-- drop helper objects if they exist
IF OBJECT_ID('dbo.vw_Test') IS NOT NULL
BEGIN
DROP VIEW dbo.vw_Test
...
```sql title="Use explicit column names"
-- create view against the table
ALTER VIEW dbo.vw_Test
AS
  SELECT A, B
...
```sql title="Add column E and select"
ALTER TABLE dbo.tb_View_Test
  Add E INT NULL
GO
...
```sql title="ALTER view to include E"
ALTER VIEW dbo.vw_Test
AS
  SELECT A, B, E
    FROM dbo.tb_View_Test
...
```sql title="Drop B, add C, and select"
ALTER TABLE dbo.tb_View_Test
DROP COLUMN B
GO
...
```sql title="Fix view with correct columns"
ALTER VIEW dbo.vw_Test
AS
  SELECT A, E, C
    FROM dbo.tb_View_Test
