...
```csharp title="RobotsTxtController.cs"
...
```csharp title="Return 404 alternative"
public ActionResult Index()
{
    var contentBuilder = new System.Text.StringBuilder();
    contentBuilder.AppendLine("User-agent: *");
...
```csharp title="Toggle robots.txt mode"
...
```xml title="Web.config appSetting"
...
```csharp title="Read config setting"
