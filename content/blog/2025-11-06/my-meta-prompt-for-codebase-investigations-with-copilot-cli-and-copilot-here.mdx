...
To do that, I use a "meta-prompt" that I keep in my clipboard history (I use [Paste](https://pasteapp.io/)) so it's always at hand. It's a template that tells GitHub Copilot CLI *exactly* how to behave, what to produce, and where to put it.

In this post, I'll share the exact prompt, the `copilot_yolo` mode I use to run it, and the kind of results you can expect. `copilot_yolo` is a more flexible investigation mode that's part of my broader `copilot_here` tool, which you can find on [GitHub](https://github.com/GordonBeeming/copilot_here).
...
This "meta-prompt" works because it's not a question, it's a set of commands:
...
3. It demands a specific report structure: `high level summary`, `detailed findings`, and `recommendations`.
...
That's where my `copilot_here` project comes in. For these deep investigations, I use the `copilot_yolo` mode. It's a "yolo version" that's still part of the `copilot_here` toolset, but it gives Copilot more flexibility and access to all tools to work through the investigation.
...
This makes for a clean workflow:

  * The AI can write its findings.
  * The report lives in a central `~/investigations` folder on my host machine.
  * It doesn't pollute my project's `git status` with a large, uncommitted markdown file.
...
After a few follow-up questions, Copilot had all the context it needed. It generated the file `~/investigations/20251106-SSW.TimePRO-Xero-Integration-Configuration-And-Interactions.md`.

The report was comprehensive in a way that genuinely surprised me. It saved me hours of manual digging.
...
This one stood out. It found a "TODO" comment in the codebase and correctly identified it as a high-priority performance issue worth acting on.
...
This workflow has changed how I approach codebase analysis. A few things I'd take away from it:

  * Don't just ask simple questions. Tell the AI *how* to answer you and *what* to produce.
  * Pair your prompt with a sandboxed tool like `copilot_yolo` to give the AI the power it needs while keeping file access under control.
  * You can grab `copilot_here` (which includes `copilot_yolo`) from the [copilot_here website](https://copilot_here.gordonbeeming.com).
  * For the global mounts and path-mapping features, check out the [full `copilot_here` November 2025 update post](https://www.google.com/search?q=https://gordonbeeming.com/blog/2025-11-30/copilot-cli-in-docker-november-2025-updates-better-context-and-flexible-mounts).
