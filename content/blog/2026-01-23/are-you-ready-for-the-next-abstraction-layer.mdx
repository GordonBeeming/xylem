...
I was grabbing a coffee yesterday with [Sam Wagner](https://www.linkedin.com/in/sam-b-wagner/) and our conversation really sparked my thinking on this topic, helping it form a little more concretely (at least for today 🥴) than it has been for a while.

How old were you when you realized you don't actually care how your email gets sent?
...
A decade or two ago, you might have tried to "roll your own" auth. You'd manage the salt, the hashing, if you even did that, or whatever other "security" measures you had in place. Today? You'd be called crazy for not using a proven provider or a battle-tested library.
...
Let's look at SMTP (Simple Mail Transfer Protocol) as our guide to see how we got here.
...
If you missed a character or the server timed out, you were stuck. You weren't just "sending mail"; you were managing a state machine over a socket.
...
Then package management became popular.

Suddenly there were SMTP packages on the internet written by people we didn't know. I remember the pushback: *"We shouldn't just use some random person's code in a black box! If we write it ourselves, we can maintain it."*
...
Once this mindset took hold, open source really stepped in to carry the load. Because the code for these packages was "out there" and transparent, we felt we could finally trust the abstraction. We stopped looking at the raw SMTP logs (and yes I did my fair share of that) because we had more important business problems to solve. Sure, maybe we trusted it a bit too blindly at times, but the productivity gains were real.

### 3. The API and SaaS era (total decoupling)

Today, we've moved even further. We use services like SendGrid. We don't even care about transient errors, managing mail servers, or deliverability anymore. We send a JSON object over a simple HTTP request, and the SaaS provider handles the heavy lifting: tracking bounces, managing IP reputations, and giving us fancy graphs of our success rates.
...
Low-code platforms tried to sell us this dream for years. The problem was they always hit a "complexity ceiling." As soon as you needed something custom, the abstraction leaked, and you were back to writing "real" code.
...
We've just come off the back of an era where this stuff was *almost* but not quite good enough. For a while, AI felt like a fancy autocomplete that still required you to babysit every semicolon. Many people still think we're stuck in that "uncanny valley" where the effort to fix AI's mistakes is higher than just writing it yourself.

What's different now? It's the move from pattern matching to reasoning. Earlier iterations were just guessing the next word. Today's models, especially when paired with the deterministic tools of MCP, aren't just guessing; they are navigating logic. We've moved past the "close enough" phase into the "functional outcome" phase.

## Making AI Deterministic: The Role of MCP

So, how do we make AI agents actually "good" teammates? How do we keep them focused on business value without getting lost in the weeds?
...
The .NET SDK is essentially the same concept, just on a much larger scale. When you look at the thousands of method signatures in .NET, they are effectively a massive collection of tool definitions.
...
But we're heading toward a future where large (and effective) context windows, combined with smarter ways to index tools, will change that.
...
This works even better when combined with sub-agents for context management. Instead of one agent trying to "know" everything, you use a swarm where each specialist operates in its own clean context window. Each sub-agent gets a specific set of tools and only the information it needs for its sub-task.

To be clear: I'm not talking about some "AI slop" landing in a codebase. I'm saying the code written would be as if you had a real specialist for every part of the codebase and .NET framework, each handing work to the next. They get out of each other's way, and yes, I mean that genuinely.

We're moving toward a world where you don't spend your day wrestling with syntax or boilerplate. You're thinking about what the code needs to do and whether it does it correctly. Security and best practices aren't "extra" steps you bolt on at the end; they just come out the other side. Your job shifts from writing lines to being the judge of the output. You verify the results, decide if they meet the bar, and ship it.
...
History says the abstraction always wins because productivity and business value always win. 🏁
