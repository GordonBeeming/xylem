...
After that, whenever you change a config, the change happens in `~/dotfiles/` (because that's where the symlink points), so you just commit and push. New machine? Clone the repo and run `stow .`.
