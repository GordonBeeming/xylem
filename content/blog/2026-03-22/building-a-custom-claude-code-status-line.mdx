...
The tricky part was GitButler detection. I use [GitButler](https://gitbutler.com/) for branch management, which keeps you on a `gitbutler/workspace` branch while your actual virtual branches live inside GitButler's system. A normal `git branch --show-current` would just say "gitbutler/workspace" — not helpful.
