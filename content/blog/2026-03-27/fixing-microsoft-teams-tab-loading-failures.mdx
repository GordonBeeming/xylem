...
<Figure key="/images/a3b7c9e1f4d2468ab5c0e8f3d7a1b6c2.png" src="/images/a3b7c9e1f4d2468ab5c0e8f3d7a1b6c2.png" alt="Teams tab showing 'If you're having issues with this app, try the desktop version' error with 'failed reason: Ocdi' in the browser console" width="0" height="0" caption="The Teams tab fails to load, showing an error message (1) and logging 'failed reason: Ocdi' in the console (2)" />
...
In our case, the app's infrastructure (Bicep/ARM template) was setting the tab's `contentUrl` to the raw Azure Blob Storage URL instead of the custom domain. The blob storage URL had no CSP headers at all, while the custom domain (behind Cloudflare) had the correct `frame-ancestors` policy.
